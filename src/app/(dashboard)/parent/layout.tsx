...
const navItems = [
  ...
  { href: '/parent/change-password', label: 'Change Password' },
]
...
export default function ParentLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!session || session?.user?.role !== Role.PARENT) return null
    ...
    <div className="min-h-screen flex bg-gray-50 dark:bg-gray-950">
      <aside className="w-64 bg-white dark:bg-gray-900 border-r dark:border-gray-800 shrink-0 hidden md:flex flex-col">
        <div className="p-6 border-b dark:border-gray-800">
          <Link href="/parent/dashboard" className="text-xl font-bold text-blue-600 dark:text-blue-400">
            Parent Portal
          </Link>
        </div>
